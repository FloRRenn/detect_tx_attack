...
class NodeAttention(nn.Module):
    ...
    def forward(self, x_in, x_node_eoa, x_node_dapp):
        batch_size = x_in.size(0)
        seq_length = x_in.size(1)
        softmax = nn.Softmax()

        x_node = torch.cat((x_node_eoa, x_node_dapp), -1)
        ...
        c = LinearF.apply(torch.cat((x_node.view(-1, x_node.size(2)).unsqueeze(1), embed_e), -1), self.weight_ih, self.bias_ih)
        c = torch.sigmoid(c)
        ones_tensor = torch.ones(batch_size * seq_length, 1, 1).to(c.device)
        
        output_left = (ones_tensor - c).bmm(embed_e.to(c.device))
...
class LSTMTagger(nn.Module):
    def __init__(self, embedding_dim, hidden_dim, vocab_size, target_size):
        ...
        self.log_softmax = torch.nn.Softmax()
    ...
    def forward(self, list_int, batch_size, x, y = None, x_node_eoa = None, x_node_dapp = None):
        self.PAD_VALUE  = 0
        
        input_emb = self.attention(x, x_node_eoa, x_node_dapp)
        input_emb = input_emb.permute(1, 0, 2)

        input_emb = list(itertools.zip_longest(*(input_emb.detach().cpu().numpy().tolist()), fillvalue = -1))
        input_emb = torch.FloatTensor(input_emb).to('cuda')
        ...
        tags = self.log_softmax(self.hidden2tag(out)).to('cuda')
        ...
        if y is not None:
            ...
            loss = self.cirterion(tags, y_onehot.to('cuda'))
            pred = tags.data.max(1, keepdim=True)[1].cpu()
            acc = pred.eq(y.data.view_as(pred)).sum().float() / float(y.size()[0])
